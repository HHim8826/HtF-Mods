...
轉到 94.0 就是那首」，就得真的把頻道加出來——`頻道 / 頻道數` 設 **0（自動）**
就是一首一個頻率，頻率平均攤在 88–108 上。
...
- **頻道之間要夠遠。** `ApplyVolume` 算的是
  `Clamp01(1 - (|目前頻率 - 頻道頻率| - 0.5))`——誤差 0.5 以內滿音量，到 1.5 才完全消失。
  所以兩台相距 **3.0** 才聽得乾淨；更密就會互相滲音。88–108 共 20，
  **最多放得下 7 台**。自動模式會照 `最小頻道間距` 夾住上限；歌比這多就回到
  「一個頻道排幾首、自動接著播」。硬要塞更多就直接把 `頻道數` 設成數字，
  log 會警告會滲音但照做。
- **這是本機的東西。** 頻道是在本機生出來的，同步的只有「你轉到幾點幾」這個數字。
  別人沒裝、或設定的頻道數不一樣，轉到同一個頻率聽到的就不是同一台。
  要和別人一起聽，大家的 `頻道數`、`最小頻道間距` 和音樂資料夾都要一致。
