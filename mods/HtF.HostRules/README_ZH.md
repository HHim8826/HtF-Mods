...
**預設關閉**——它拿掉的是遊戲的核心懲罰，要自己開。

值得知道它關掉了什麼：遊戲原本的規則是，**全員陣亡**時那次重生會讓**每一位玩家**都掉一次，
不是只有放棄的那個人。
...
遊戲掉東西走的是兩條互不相干的路，房主端只搆得到其中一條：

| 發生什麼 | 遊戲做了什麼 | 房主改得到嗎 |
|---|---|---|
| 放棄、重生 | `PlayerInventory.ServerDropAll` 掉手上的**加整個背包** | 可以，就是上面那個設定 |
| 被打倒、變成可被救起的身體 | `PlayerDying.ServerDie` 放掉**手上那一件** | **不行** |

第二條不是漏掉。倒下的人**自己的客戶端**會跑 `PlayerDying.LocalDie`，那一行不問伺服器就把
手上的東西丟出去，房主端的 mod 搆不到別人機器上的那一行。硬讓伺服器留著物品的話，
它會卡在一個「別人撿不起來、自己也拿不回來」的狀態——**比讓它掉在地上還糟**。
完整的推導寫在 `DeathRules.cs` 的類別註解裡，免得有人再試一次同一招。
