...
`HtF.Economy` 的釣魚那半搬到這裡了。它本來就不屬於「經濟」：抽魚權重、保底、咬鉤時間
都是**房主替整個房間定的規則**，生效條件跟難度乘數一模一樣——放在另一個 mod 只是讓房主
要開兩個設定頁面調同一件事。

**`HtF.Economy` 或 `HtF.FishingEcology` 還裝著的話請刪掉。** 三者都用同樣的方式 patch
`CreatureManager.GetRandomItem`，兩份同時載入會讓倍率**疊乘**。咬鉤時間更糟：兩邊各自對
`BaitInfo._catchTimeMinMax` 做快照再乘，後套用的那個會把**已經被改過的值**當成原值存起來，
之後連還原都還原不回去。兩種情況都不會有任何錯誤訊息。

偵測到的時候，這個 mod 會**自動停用自己的釣魚那半**並在 log 說一聲，這樣至少不會疊乘。
難度、規則、玩家數值不受影響。把舊的資料夾刪掉重開遊戲，釣魚就會回來。

`HtF.Economy` 的金錢那半——賣價倍率、花費倍率、新存檔起始金錢——是**移除**，不是搬家。
本來在用那三個的話，這一版之後就沒有了。

舊的 `htf.economy.cfg` 不會沿用：插件 GUID 不同，BepInEx 會另外開一份新的。
設定名稱沒有改，重填很快。
